...
# Save plot
path = "/Users/janekb/Documents/learn/iv/paper/figures/"
pdf(file = file.path(path, "sim_cov.pdf"), width = 8.5, height = 6)
model_df_corr %>% 
...
# save plot
pdf(file.path(path, "sim_n.pdf"), width = 8.5, height = 6)
model_df_nobs %>%
  filter(term == "x1" | term == "xHat") %>%
...
# save plot
pdf(file.path(path, "sim_weak_iv.pdf"), width = 8.5, height = 6)
model_df_weak_iv %>%
  filter(term == "x1" | term == "xHat") %>% 
  filter(model == "2SLS") %>%
